...
async function postQuestion(req, res) {
  try {
    let { title, question_description, user_id, tag } = req.body;

    // Input Validation
    if (!title || !question_description || !user_id || !tag) {
      return res.status(StatusCodes.BAD_REQUEST).json({
        msg: "Please provide title, description, user_id, and tag.",
      });
    }
    ...
    // Insert into database (keeping Quill's formatting intact)
    const query = `
      INSERT INTO questionTable (question_id, user_id, title, question_description, tag)
      VALUES (?, ?, ?, ?, ?)
    `;
    ...
    const [result] = await dbConnection.query(query, [
      ...
      user_id,
      title,
      question_description, // Keep formatting!
      tag,
    ]);

    return res.status(StatusCodes.CREATED).json({
      msg: "Question created successfully",
      question_id, // Use UUID, not MySQL insertId
    });
  } catch (error) {
